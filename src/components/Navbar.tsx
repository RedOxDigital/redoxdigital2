...
const FullScreenMenu = ({ isOpen }: { isOpen: boolean }) => (
  <AnimatePresence>
    {isOpen && (
      <motion.div
        initial={{ y: "-100%" }}
      ...
      >
        <div className="flex flex-col gap-6 text-center">
          {["The Hub", "Strategy", "Case Studies", "Contact"].map((item, i) => (
            <motion.div
              key={item}
            ...
            >
              <a href="#" className="font-syne text-5xl md:text-8xl font-bold uppercase hover:text-[#E02020] transition-colors duration-300">
                {item}
              </a>
            </motion.div>
          ))}
